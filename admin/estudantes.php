<?php
require '../config.php';
$sql_pag = "SELECT * FROM estudantes";
$sql_pag = $pdo->query($sql_pag);
$total = $sql_pag->rowCount();
if(empty($_GET['pag'])){
    $pag=1;
}else{
...
$maximo=30; //Quantidade Máxima de posts por página
$inicio = ($pag * $maximo) - $maximo; //Variável para LIMIT da sql
$paginas=ceil($total/$maximo);	//Quantidade de páginas
?>
    ...
    <body>
        ...
        <div id="caixa_preta">
            ...
            <div id="box_aluno">
                ...
                <?php
                echo "<h1>Página ". $pag." de ".$paginas."</h1>";
                $sql = "SELECT * FROM estudantes WHERE nome != '' ORDER BY nome LIMIT $inicio, $maximo";
                $sql = $pdo->query($sql);
                        ...
                        <?php foreach ($sql as $alu): ?>
                            <tr>
                                ...
                                <td><a class="a" href="estudantes.php?pg=todos&func=delete&id=<?php echo $alu['id']; ?>&code=<?php echo $alu['code']; ?>">
                                    ...
                                    <a class="a" target="_blank" href="relatorio_boletim_escolar.php?id=<?php echo $alu['code']; ?>&code=<?php echo $alu['code'];?>"><img title="Boletim" src="../img/boletim.png" width="18" height="18" border="0"></a>
			...
			<?php
				if($pag!=1){
					echo "<td><h3><a class='pagin' href='estudantes.php?pag=".($pag-1)."'> Página Anterior</a></h3></td>";
				}
				if($total<=$maximo){
					...
					for($i=1;$i<=$paginas;$i++){
						if($pag==$i){
							echo "<td class='pagin1'><h3><a class='pagin' href='estudantes.php?pag=".$i."'> ".$i."</a></h3></td>";
						}else{
							echo "<td><h3><a class='pagin' href='estudantes.php?pag=".$i."'> ".$i."</a></h3></td>";
						}
					}
				}
				if($pag!=$paginas){
					echo "<td><h3><a class='pagin' href='estudantes.php?pag=".($pag+1)."'> Próxima Página</a></h3></td>";
				}
			?>
		</tr>
		</table>
                            ...
                            <tr>
                                ...
                                <td><label for="turno"></label>
                                    <select name="turno" size="1" id="turno">
                                        ...
                                        <option></option>
                                        <?php
                                        $sql_5 = "SELECT * FROM periodos";
                                        $sql_5 = $pdo->query($sql_5);
                                        foreach ($sql_5 as $res_5): ?>
                                        <option value="<?php echo $res_5['periodos']; ?>"><?php echo $res_5['periodos'];?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </td>
                            ...
                            <tr>
                                ...
                                <td><label for="turno"></label>
                                    <select name="turno" size="1" id="turno">
                                        ...
                                        <option value="<?php echo $res_3['turno']; ?>"><?php echo $res_3['turno']; ?></option>
                                        <?php endforeach; ?>
                                    </select></td>
                                <td><label for="atendimento_especial"></label>
